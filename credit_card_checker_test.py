...
class CreditCardCheckerTest(unittest.TestCase):
    def test_single_digit_strings_can_not_be_valid(self):
        self.assertIs(CreditCardChecker("1").valid(), False)

    def test_a_single_zero_is_invalid(self):
        self.assertIs(CreditCardChecker("0").valid(), False)

    def test_a_simple_valid_sin_that_remains_valid_if_reversed(self):
        self.assertIs(CreditCardChecker("059").valid(), True)

    def test_a_simple_valid_sin_that_becomes_invalid_if_reversed(self):
        self.assertIs(CreditCardChecker("59").valid(), True)

    def test_a_valid_canadian_sin(self):
        self.assertIs(CreditCardChecker("055 444 285").valid(), True)

    def test_invalid_canadian_sin(self):
        self.assertIs(CreditCardChecker("055 444 286").valid(), False)

    def test_invalid_credit_card(self):
        self.assertIs(CreditCardChecker("8273 1232 7352 0569").valid(), False)

    def test_invalid_long_number_with_an_even_remainder(self):
        self.assertIs(CreditCardChecker(
            "1 2345 6789 1234 5678 9012").valid(), False)

    def test_valid_number_with_an_even_number_of_digits(self):
        self.assertIs(CreditCardChecker("095 245 88").valid(), True)

    def test_valid_number_with_an_odd_number_of_spaces(self):
        self.assertIs(CreditCardChecker("234 567 891 234").valid(), True)

    def test_valid_strings_with_a_non_digit_added_at_the_end_become_invalid(self):
        self.assertIs(CreditCardChecker("059a").valid(), False)

    def test_valid_strings_with_punctuation_included_become_invalid(self):
        self.assertIs(CreditCardChecker("055-444-285").valid(), False)
